...
public class RemovableFrame extends MainFrame implements MouseListener, KeyListener {
	/**
	 * 
	 */
	...
	public RemovableFrame() {
		...
		String infor = "<html><body>" + "<font color=#FFFFFF>欢迎！</font>" + "<font color=black><b>" + user.getUserName()
				+ "</font>" + "</body></html>";
		...
		listener = new CheckRefresh();	
		listener.start();//采用线程休眠技术错开时间，形成时间差，从而避免了因一写入就刷新而导致的SQLite死锁

	}

	public JScrollPane setSearchTree() {
		UserDao cmd = Instance.getGeneral();
		...
		tree.addTreeSelectionListener(new TreeSelectionListener() {
			...
			public void valueChanged(TreeSelectionEvent arg0) {
				if (!tree.isSelectionEmpty()) {
					...
					UserDao data = Instance.getGeneral();
	...
	public BGPanel getContentPane(String title) {
		if (backPanel == null) {
			...
			backPanel.setBorder(createTitledBorder(null, title, TitledBorder.DEFAULT_JUSTIFICATION, TitledBorder.TOP,
					new Font("楷书", Font.BOLD, 17), Color.YELLOW));
			...
			public void actionPerformed(ActionEvent arg0) {
				if (arg0.getActionCommand().equals("搜索")) {
					if (!isLegal(cmdTextField.getText())) {
						JOptionPane.showMessageDialog(getContentPane(), "您输入的命令不规范：为空或包含空格！", "命令非法",
								JOptionPane.WARNING_MESSAGE);
						cmdTextField.requestFocus();
						cmdTextField.setText("");

					} else if (cmdTextField.getText().matches("[0123456789]+")) {
						JOptionPane.showMessageDialog(getContentPane(), "您输入的命令不正确：命令不能纯数字！", "命令非法",
								JOptionPane.WARNING_MESSAGE);
						cmdTextField.requestFocus();
						cmdTextField.setText("");
					} else {
						...
						if (!data.isExist(rst)) {
							JOptionPane.showMessageDialog(getContentPane(), "抱歉！未能搜索到此命令", "命令不存在",
	...
	public BGPanel getJPanel() {
		if (jPanel == null) {
			...
			jPanel.setBorder(createTitledBorder(null, null, TitledBorder.DEFAULT_JUSTIFICATION, TitledBorder.TOP,
					new Font("楷书", Font.BOLD, 17)));
	...
	private GlassButton getStudyButton() {
		if (StudyButton == null) {
			...
			StudyButton.setIcon(new ImageIcon(getClass().getResource("/com/zhl/button/1big.png"))); // 定义按钮的初始化背景
	...
	private GlassButton getAllcmdButton() {
		if (AllcmdButton == null) {
			...
			AllcmdButton.setIcon(new ImageIcon(getClass().getResource("/com/zhl/button/2.png"))); // 定义按钮的初始化背景
	...
	public void mousePressed(MouseEvent e) {
		...
		if (sbtn == StudyButton) {
			StudyButton.setIcon(new ImageIcon(getClass().getResource("/com/zhl/button/1big.png")));
			AllcmdButton.setIcon(new ImageIcon(getClass().getResource("/com/zhl/button/2.png")));
			sbtn.setPaintFlag(true);
		} else if (sbtn == AllcmdButton) {
			StudyButton.setIcon(new ImageIcon(getClass().getResource("/com/zhl/button/1.png")));
			AllcmdButton.setIcon(new ImageIcon(getClass().getResource("/com/zhl/button/2big.png")));
			sbtn.setPaintFlag(true);
		} else {
		}
		repaint();
	...
	class ToolsButtonActionAdapter implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getSource() == StudyButton) {
				...
				backPanel.removeAll();
				panel_1.add(command);
				backPanel.setBorder(createTitledBorder(null, "学习旅程", TitledBorder.DEFAULT_JUSTIFICATION,
						TitledBorder.TOP, new Font("楷书", Font.BOLD, 17), Color.YELLOW));
				backPanel.add(tablePanel);
				backPanel.add(cmdLabel);
				backPanel.add(tablePanel);
				backPanel.add(cmdTextField);
			...
			if (e.getSource() == AllcmdButton) {
				panel_1.removeAll();
				backPanel.removeAll();
				backPanel.setBorder(createTitledBorder(null, null, TitledBorder.DEFAULT_JUSTIFICATION, TitledBorder.TOP,
						new Font("楷书", Font.BOLD, 17), Color.YELLOW));
				// panel_1.add();
				String infor = "<html><body>" + "<font color=white><b>" + "敬请期待！" + "</font>" + "</body></html>";
	...
	public void keyPressed(KeyEvent arg0) {
		if (arg0.getKeyCode() == KeyEvent.VK_ENTER) {
			if (!isLegal(cmdTextField.getText())) {
				...
				String rst = cmdTextField.getText();
				if (!data.isExist(rst)) {
					JOptionPane.showMessageDialog(getContentPane(), "抱歉！未能搜索到此命令", "命令不存在", JOptionPane.ERROR_MESSAGE);
					cmdTextField.requestFocus();
					cmdTextField.setText("");
				} else {
					String str[] = data.getCmd(rst);
	...
	class CheckRefresh extends Thread {
