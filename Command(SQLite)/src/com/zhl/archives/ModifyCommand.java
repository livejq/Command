...
public class ModifyCommand extends JFrame {
	...
	private String cata[] = { "文件管理", "文件传输", "文档编辑", "磁盘管理", "磁盘维护", "网络通信", "系统管理", "系统设置", "备份压缩", "其它", "X-Window",
			"电子邮件与新闻组", "附录" };
	...
	public ModifyCommand() {
		...
		modifyButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (!dao.isExist(cmd)) {
					JOptionPane.showMessageDialog(getContentPane(), "抱歉！此命令不存在", "警告", JOptionPane.WARNING_MESSAGE);
				} else {
					...
					if (dao.doModify(provide)) {
						JOptionPane.showMessageDialog(getContentPane(), "恭喜！修改成功", "修改成功", JOptionPane.DEFAULT_OPTION);
						...
						JOptionPane.showMessageDialog(getContentPane(), "无法修改此命令！", "错误提示", JOptionPane.ERROR_MESSAGE);
					}
				}
			}
		});
